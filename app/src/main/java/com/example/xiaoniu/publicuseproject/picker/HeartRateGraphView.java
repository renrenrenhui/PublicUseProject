...
//                mCurvePath.cubicTo((endPointX + startPointX) / 2, startPointY, (startPointX + endPointX) / 2, endPointY, endPointX, endPointY);
    ...
    private void init(Context context) {
        mPaddingTop = dip2px(context, 5);
        mPaddingStart = dip2px(context, 12);
        mPaddingEnd = dip2px(context, 12);
        mPaddingBottom = dip2px(context, 20);

        //TODO 基础心率先不显示，等显示再设置回来，不然view的高度太多空白
        mTopTextSize = sp2px(context, 0);
        mTopTextPaddingBottom = dip2px(context, 12);
...
//        mCurvePaint.setShader(lg);
        ...
        mTopTextPaint.setColor(getResources().getColor(R.color.white_tx_color));
    }
