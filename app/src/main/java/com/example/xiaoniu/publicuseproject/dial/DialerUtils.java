...
//                    JumpPermissionManager.openAppSettings(context);
//                    return;
//                }
                ...
                // modify by genius
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M /*&& PermissionHelper.hasPermissions(context, Manifest.permission.CALL_PHONE)*/) {
    ...
    public static boolean isRtl(Context context) {
        boolean isrtl = false;
        try {
            int type = getLayoutDirection(context);
            isrtl = (type == View.LAYOUT_DIRECTION_RTL);
        } catch (Exception e) {
        } finally {
            return isrtl;
        }
